...
def constructedGraph(filename1,knowcomcom):
    ...
    sp22 = sp.transform(sp_graph) #两两已知社区计算相似性 两维数组形式存储的
    sp2=np.nan_to_num(sp22)
    mean = []
    for i1 in range(len(sp2)):
        for i2 in range(i1+1,len(sp2)):
            mean.append(sp2[i1][i2])#获取数组上三角以上的元素
    inde = math.ceil(len(mean) / 2) #计算出中位数
    value_inde = sorted(mean)[inde] #对mean中相似性值进行排序，取出中位数的值
    G4=nx.Graph() #构建一个无向带权图
    for i in range(len(sp2)):
        for j in range(i + 1, len(sp2)):
            if sp2[i][j]>=value_inde:
                G4.add_edge(i,j,weight=sp2[i][j])
    #G4是一个无向带全图，value_inde是中位数值，fenmu是数组的最大值减去最小值
   #nmin是数组的最小值，com10是10个已知社区，sp_graph,10个已知社区表示的最短路径图
...
def community_detection(C1,N1,local_com,G,global_com,maxsize,quanzhong):
    ...
    while True:
        ...
        for nodev in N:
            ...
            if Snew >= np.nan_to_num(np.mean(global_used_similarity)) and Mnew >= Mmax:#一开始社区历史相似性列表为空，对空列表求均值的时候会出现nan
...
def community_detection1(C1,N1,local_com,G,global_com,maxsize,quanzhong,localsmi):
...
def Go(para):
    i = para[0]
    filename, knowcomcom,file, G4,value_inde,com10,sp_graph,maxsize1= para[1], para[2], para[3], para[4], para[5],para[6],para[7],para[8]
    f = open(file + str(i), "w", encoding="utf-8")
    G = Function.networkx(filename)
    C11=M(i,G) #通过M方法获得种子节点所在的社区
    G5,com10,value11=constructG(C11,filename,G4,com10,sp_graph,value_inde)
    #获得新的权重图
    ...
    C2, N2,ss = community_detection(C1, N1, local_com, G,global_com,maxsize1,quanzhong) #社区相似性阶段
    C, N = community_detection1(C2, N2, local_com, G, global_com, maxsize1, quanzhong,ss)  #节点相似性阶段
    f.writelines(str(C) + "\n")
    f.close()

if __name__ == '__main__':
    amazon_seed, dblp_seed, facebook_seed, twitter_seed, lj_seed = Function.seed()#数据集的种子节点
    file1 = 'facebook' #创建文件目录
    file_knowcom='dataset/facebook_knowcom'#给定的已知社区文件路径
    facebook=Function.get_knowcom(file_knowcom)
    for i in range(1, 6):#五组已知社区
        start = datetime.datetime.now()
        knowcomcom = facebook[i - 1] #数据集对应的已知社区
        filename = 'dataset/facebook-1.90.ungraph.txt'#数据集对应边的路径
        nodelist = facebook_seed  # 获取数据集的种子节点
        savefile = file1 + "/" + str(i) + "/" #把每个节点找到的社区写入文件中
        G44, value_inde, com10, sp_graph ,maxsize1= constructedGraph(filename, knowcomcom)  # 构建已知社区的相似性图
        plist = [[node, filename, knowcomcom, savefile, G44, value_inde, com10, sp_graph,maxsize1] for node in nodelist]
